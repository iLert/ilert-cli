...
//
// This defines `const EMBEDDED: &[(&str, &str)]`, one `(name, document)` pair
// per `skills/*/SKILL.md`, generated by `build.rs` on every build. Adding a
// skill therefore means adding its directory and nothing else.
//
...
// catalog and the content cannot disagree. `build.rs` regenerates the manifest
// from that same frontmatter, and `manifest_agrees_with_the_skills` fails if
// what is committed has fallen behind.
...
/// single-line, and `build.rs` reads it with the same rules.
...
mod tests {
    ...
    /// `build.rs` rewrites it, so a failure here means the regenerated file was
    /// never committed, or could not be written at all.
    #[test]
    fn manifest_agrees_with_the_skills() {
        ...
        assert_eq!(
            ...
            "skills/index.json lists {} skills but {} are embedded — \
             commit the regenerated skills/index.json",
            listed.len(),
            EMBEDDED.len()
        );
